...
import { EnvironmentalCalculatorService } from './environmentalCalculator.js';

export class CertificateService {
  private static BASE_URL = process.env.PUBLIC_APP_URL || 'http://localhost:5173';
  ...
  static async generateCertificate(params: {
    ...
    if (booking.status !== 'COMPLETED') {
      throw new Error(
        `Certificate can only be issued for COMPLETED recycling transactions. Current status is ${booking.status}.`
      );
    }
    ...
    const envImpact = EnvironmentalCalculatorService.calculate(weightKg, categorySlug);
  ...
  static async getAdminCertificateSummary(params: {
    ...
    const activeCertificates = await prisma.certificate.count({ where: { certificateStatus: 'VERIFIED' } });
    const revokedCertificates = await prisma.certificate.count({ where: { certificateStatus: 'REVOKED' } });
