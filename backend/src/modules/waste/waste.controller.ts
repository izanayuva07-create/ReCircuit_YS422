...
wasteRouter.post('/analyze', async (req: Request, res: Response) => {
  try {
    const imageUrl =
      req.body.imageUrl ||
      req.body.image ||
    ...
    const aiResult = await AIService.classifyImage(imageUrl, hint);
    ...
    res.json({
      ...
      data: {
        ...
        safetyWarnings:
          aiResult.topPrediction.attributes?.hazard_level === 'CRITICAL'
            ? [
...
wasteRouter.get('/categories', async (req: Request, res: Response) => {
  res.json({
    success: true,
    data: [
      'mobile',
...
wasteRouter.get('/listings', async (req: Request, res: Response) => {
  try {
