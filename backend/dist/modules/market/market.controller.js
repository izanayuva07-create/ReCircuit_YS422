...
// ==========================================
// GET /listings/my
exports.marketRouter.get('/listings/my', async (req, res) => {
    try {
...
// GET /listings/:id
exports.marketRouter.get('/listings/:id', async (req, res) => {
    try {
        const l = await prisma_js_1.prisma.listing.findUnique({
        ...
        res.json({
            ...
            data: {
                id: l.id,
                sourceId: l.sourceId,
                category: (l.category?.aiClassId?.includes('phone') ? 'mobile' : 'laptop'),
                itemName: l.title,
                quantity: 1,
                weightKg: l.estimatedWeight || 2.0,
                condition: 'working',
    ...
    try {
        ...
        const listing = await prisma_js_1.prisma.listing.create({
            data: {
                ...
                images: JSON.stringify(images && images.length > 0
                    ? images
...
// PUT /listings/:id
exports.marketRouter.put('/listings/:id', async (req, res) => {
    try {
        const updated = await prisma_js_1.prisma.listing.update({
            where: { id: req.params.id },
            data: {
...
// DELETE /listings/:id
exports.marketRouter.delete('/listings/:id', async (req, res) => {
    try {
        await prisma_js_1.prisma.listing.delete({ where: { id: req.params.id } });
...
exports.marketRouter.get('/bids/listing/:listingId', async (req, res) => {
    try {
...
exports.marketRouter.get('/bids/my', async (req, res) => {
    try {
        const bids = await prisma_js_1.prisma.bid.findMany({
...
// POST /bids
exports.marketRouter.post('/bids', async (req, res) => {
    try {
        const { listingId, offeredPrice, notes } = req.body;
        const col = await prisma_js_1.prisma.collector.findFirst();
        const bid = await prisma_js_1.prisma.bid.create({
            data: {
                listingId,
...
// PUT /bids/:id/accept
exports.marketRouter.put('/bids/:id/accept', async (req, res) => {
    try {
...
// PUT /bids/:id/reject
exports.marketRouter.put('/bids/:id/reject', async (req, res) => {
    try {
        const bid = await prisma_js_1.prisma.bid.update({
            where: { id: req.params.id },
...
// ==========================================
// GET /bookings/my
exports.marketRouter.get('/bookings/my', async (req, res) => {
    try {
        ...
        const items = bookings.map((b) => ({
            ...
            status: (b.status.toLowerCase() === 'booked' ? 'confirmed' : b.status.toLowerCase()),
            otp: '7842',
...
// GET /bookings/:id
exports.marketRouter.get('/bookings/:id', async (req, res) => {
    try {
        const b = await prisma_js_1.prisma.booking.findUnique({
            where: { id: req.params.id },
...
exports.marketRouter.post('/bookings/:id/verify-otp', async (req, res) => {
    try {
        const { otp } = req.body;
...
exports.marketRouter.put('/bookings/:id/status', async (req, res) => {
    try {
        const { status } = req.body;
        const b = await prisma_js_1.prisma.booking.update({
            where: { id: req.params.id },
...
exports.marketRouter.get('/collector/profile', async (req, res) => {
    try {
        const col = await prisma_js_1.prisma.collector.findFirst({ include: { user: true } });
        res.json({
            success: true,
            data: {
                id: col?.id || 'col-001',
...
exports.marketRouter.get('/collector/jobs/available', async (req, res) => {
    try {
        const listings = await prisma_js_1.prisma.listing.findMany({
        ...
        const jobs = listings.map((l, idx) => ({
            ...
            listing: {
                id: l.id,
                sourceId: l.sourceId,
                category: (l.category?.aiClassId?.includes('phone') ? 'mobile' : l.category?.aiClassId?.includes('pcb') ? 'pcb' : 'laptop'),
                itemName: l.title,
                quantity: 1,
...
exports.marketRouter.get('/collector/jobs/:id', async (req, res) => {
    try {
        const listingId = req.params.id.replace(/^job-/, '');
        const l = await prisma_js_1.prisma.listing.findUnique({
            where: { id: listingId },
            include: { category: true },
        });
...
exports.marketRouter.get('/collector/inventory', async (req, res) => {
    try {
        const inventories = await prisma_js_1.prisma.inventory.findMany({
            include: { category: true },
            take: 20,
        });
        const items = inventories.map((inv) => ({
            ...
            category: (inv.category?.aiClassId?.includes('phone') ? 'mobile' : inv.category?.aiClassId?.includes('pcb') ? 'pcb' : 'laptop'),
            itemName: inv.title,
            quantity: 1,
...
exports.marketRouter.post('/collector/inventory', async (req, res) => {
    try {
        const col = await prisma_js_1.prisma.collector.findFirst();
        const cat = await prisma_js_1.prisma.category.findFirst();
        const inv = await prisma_js_1.prisma.inventory.create({
            data: {
                collectorId: col?.id || 'col_1',
...
// GET /collector/lots
exports.marketRouter.get('/collector/lots', async (req, res) => {
    res.json({
        success: true,
        data: [
            {
...
// POST /collector/lots
exports.marketRouter.post('/collector/lots', async (req, res) => {
...
exports.marketRouter.get('/recycler/profile', async (req, res) => {
    res.json({
        success: true,
        data: {
            id: 'rec-001',
...
exports.marketRouter.get('/recycler/lots/incoming', async (req, res) => {
    const lots = [
        {
            id: 'lot-002',
            collectorId: 'col-001',
...
exports.marketRouter.get('/recycler/lots/:id', async (req, res) => {
    res.json({
        success: true,
        data: {
            id: req.params.id,
...
exports.marketRouter.put('/recycler/lots/:id/accept', (req, res) => {
    res.json({
        success: true,
        data: { id: req.params.id, status: 'accepted', message: 'Lot accepted for smelting manifest.' },
    });
});
// PUT /recycler/lots/:id/reject
exports.marketRouter.put('/recycler/lots/:id/reject', (req, res) => {
    res.json({
        success: true,
        data: { id: req.params.id, status: 'rejected', reason: req.body.reason || 'Composition deviation.' },
    });
});
// PUT /recycler/lots/:id/received
exports.marketRouter.put('/recycler/lots/:id/received', (req, res) => {
    res.json({
        success: true,
        data: { id: req.params.id, status: 'received', message: 'Custody transfer confirmed at weighbridge.' },
    });
});
// PUT /recycler/lots/:id/processed
exports.marketRouter.put('/recycler/lots/:id/processed', (req, res) => {
    res.json({
        success: true,
        data: {
            id: req.params.id,
...
// ==========================================
// GET /transactions/my
exports.marketRouter.get('/transactions/my', async (req, res) => {
    try {
        ...
        const items = transactions.map((t) => ({
            ...
            userId: 'user-001',
            role: 'source',
            type: t.amountCents >= 0 ? 'earning' : 'payment',
...
exports.marketRouter.get('/transactions/summary', async (req, res) => {
    try {
        const tx = await prisma_js_1.prisma.transaction.findMany();
