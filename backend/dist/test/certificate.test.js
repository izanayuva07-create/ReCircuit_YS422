...
async function runCertificateTests() {
    ...
    try {
        ...
        const sourceUser = await prisma_js_1.prisma.user.findFirst({ where: { role: 'source' } });
        const collector = await prisma_js_1.prisma.collector.findFirst();
        const category = await prisma_js_1.prisma.category.findFirst({ where: { slug: 'computers-laptops' } });
        assert(!!sourceUser && !!collector && !!category, 'Found base seed entities for certificate test');
        const testListing = await prisma_js_1.prisma.listing.create({
            data: {
        ...
        const testBooking = await prisma_js_1.prisma.booking.create({
            data: {
        ...
        const cert = await certificate_service_js_1.CertificateService.generateCertificate({
        ...
        console.log('\n🔒 Test 2: Duplicate Certificate Prevention...');
        const certDuplicate = await certificate_service_js_1.CertificateService.generateCertificate({
            bookingId: testBooking.id,
            requestUserId: sourceUser.id,
        });
        assert(certDuplicate.id === cert.id, 'Duplicate generation idempotently returns existing certificate');
        const certCountForBooking = await prisma_js_1.prisma.certificate.count({ where: { transactionId: testBooking.id } });
        ...
        const verification = await certificate_service_js_1.CertificateService.verifyCertificate(cert.certificateId);
        ...
        assert(verification.phone === undefined, 'No sensitive phone number exposed');
        assert(verification.email === undefined, 'No private email exposed');
        // Test 4: Invalid Certificate Lookup
        console.log('\n❌ Test 4: Invalid Certificate Handling...');
        const invalidCheck = await certificate_service_js_1.CertificateService.verifyCertificate('RC-EWR-NON-EXISTENT-9999');
        ...
        const revoked = await certificate_service_js_1.CertificateService.revokeCertificate(cert.certificateId, 'Material batch re-audit discrepancy');
        assert(revoked.certificateStatus === 'REVOKED', 'Status updated to REVOKED');
        assert(!!revoked.revokedAt, 'Revocation timestamp recorded');
        const verificationAfterRevocation = await certificate_service_js_1.CertificateService.verifyCertificate(cert.certificateId);
