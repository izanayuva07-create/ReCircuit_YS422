...
const matching_service_js_1 = require("../matching/matching.service.js");
const ledger_service_js_1 = require("../payments/ledger.service.js");
const constants_js_1 = require("../../lib/constants.js");
const index_js_1 = require("../../types/index.js");
exports.sourceRouter = (0, express_1.Router)();
// Helper to get effective source user
const getSourceUser = async (req) => {
    if (req.user?.id) {
    ...
    return prisma_js_1.prisma.user.findFirst({
        where: { role: 'source' },
        include: { sourceProfile: true },
    });
};
// POST /v1/source/listings (Create listing + trigger AI classify)
exports.sourceRouter.post('/listings', async (req, res, next) => {
    try {
        const user = await getSourceUser(req);
        if (!user)
            return res.status(401).json({ error: 'Source user not found' });
        // BR-007: Max 5 active listings guard
        const activeCount = await prisma_js_1.prisma.listing.count({
            where: {
                sourceId: user.id,
                status: { in: [index_js_1.ListingStatus.ACTIVE, index_js_1.ListingStatus.MATCHED, index_js_1.ListingStatus.BOOKED] },
            },
        });
        if (activeCount >= constants_js_1.BUSINESS_RULES.MAX_SOURCE_ACTIVE_LISTINGS) {
            return res.status(400).json({
                type: 'https://recircuit.org/errors/listing-limit',
                title: 'Listing limit reached',
                status: 400,
                detail: `Source cannot have more than ${constants_js_1.BUSINESS_RULES.MAX_SOURCE_ACTIVE_LISTINGS} active listings (BR-007).`,
            });
        }
        const { title, description, imageUrl, categorySlug, locationLat, locationLng, address, estimatedWeight, } = req.body;
        const img = imageUrl || 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?auto=format&fit=crop&w=800&q=80';
        // Run AI classification
        const aiResult = await ai_service_js_1.AIService.classifyImage(img, title);
        const initialStatus = aiResult.isConfident ? index_js_1.ListingStatus.ACTIVE : index_js_1.ListingStatus.LOW_CONFIDENCE;
        // Find category
...
exports.sourceRouter.get('/listings', async (req, res, next) => {
    try {
        const { all } = req.query;
        const user = await getSourceUser(req);
...
exports.sourceRouter.get('/listings/:id', async (req, res, next) => {
    try {
        const listing = await prisma_js_1.prisma.listing.findUnique({
...
exports.sourceRouter.get('/listings/:id/matches', async (req, res, next) => {
    try {
        const listing = await prisma_js_1.prisma.listing.findUnique({
            where: { id: req.params.id },
        });
        if (!listing)
            return res.status(404).json({ error: 'Listing not found' });
        const matches = await matching_service_js_1.MatchingService.findMatchingCollectors({
...
exports.sourceRouter.post('/listings/:id/book', async (req, res, next) => {
    try {
        const user = await getSourceUser(req);
        const listing = await prisma_js_1.prisma.listing.findUnique({
            where: { id: req.params.id },
        ...
        const platformFee = Math.round(price * constants_js_1.BUSINESS_RULES.PLATFORM_COMMISSION_RATE);
        // Get platform admin user for escrow hold
        const adminUser = await prisma_js_1.prisma.user.findFirst({ where: { role: 'admin' } });
        ...
        const booking = await prisma_js_1.prisma.booking.create({
            data: {
                ...
                status: index_js_1.BookingStatus.BOOKED,
        ...
        await prisma_js_1.prisma.bookingRoute.create({
            data: {
        ...
        await prisma_js_1.prisma.listing.update({
            where: { id: listing.id },
            data: { status: index_js_1.ListingStatus.BOOKED },
        });
        // Execute double-entry escrow hold
        if (user) {
            await ledger_service_js_1.LedgerService.holdEscrow({
...
exports.sourceRouter.get('/bookings', async (req, res, next) => {
    try {
        const user = await getSourceUser(req);
...
exports.sourceRouter.get('/bookings/:id/track', async (req, res, next) => {
    try {
...
exports.sourceRouter.patch('/bookings/:id/confirm-completion', async (req, res, next) => {
    try {
        const booking = await prisma_js_1.prisma.booking.findUnique({
            where: { id: req.params.id },
        ...
        const ledgerResult = await ledger_service_js_1.LedgerService.releaseEscrow({
        ...
        const updatedBooking = await prisma_js_1.prisma.booking.update({
            where: { id: booking.id },
            data: {
                status: index_js_1.BookingStatus.COMPLETED,
        ...
        await prisma_js_1.prisma.listing.update({
            ...
            data: { status: index_js_1.ListingStatus.COMPLETED },
        });
        // Increment collector total collections
        await prisma_js_1.prisma.collector.update({
...
exports.sourceRouter.post('/bookings/:id/rate', async (req, res, next) => {
    try {
        const { rating, review } = req.body;
        const booking = await prisma_js_1.prisma.booking.findUnique({ where: { id: req.params.id } });
        if (!booking)
            return res.status(404).json({ error: 'Booking not found' });
        // BR-005: Rating allowed only after COMPLETED
        if (booking.status !== index_js_1.BookingStatus.COMPLETED) {
            return res.status(400).json({
        ...
        const collectorBookings = await prisma_js_1.prisma.booking.findMany({
...
exports.sourceRouter.post('/bookings/:id/dispute', async (req, res, next) => {
    try {
        ...
        if (booking.completedAt) {
            ...
            if (hoursSinceCompletion > constants_js_1.BUSINESS_RULES.DISPUTE_WINDOW_HOURS) {
                return res.status(400).json({
                    type: 'https://recircuit.org/errors/dispute-window-closed',
                    title: 'Dispute window expired',
                    detail: `Disputes must be raised within ${constants_js_1.BUSINESS_RULES.DISPUTE_WINDOW_HOURS} hours post-completion (BR-009).`,
                });
            }
        }
        const dispute = await prisma_js_1.prisma.dispute.create({
            data: {
                bookingId: booking.id,
        ...
        await prisma_js_1.prisma.booking.update({
            where: { id: booking.id },
            data: { status: index_js_1.BookingStatus.DISPUTED },
        });
...
exports.sourceRouter.get('/wallet', async (req, res, next) => {
    try {
        const user = await getSourceUser(req);
        if (!user)
            return res.status(401).json({ error: 'Source user not found' });
        const wallet = await ledger_service_js_1.LedgerService.getOrCreateWallet(user.id);
